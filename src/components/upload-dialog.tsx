...
import { getUploadUrl } from '@/features/cos/client/cos.api'
...
export function UploadDialog({
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      setUploads((prev) =>
        prev.map((u) =>
          u.file === file ? { ...u, status: 'error' as const, error: (error as Error).message } : u
        )
      )
    }
  }
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {uploads.map((upload, index) => (
                <div
                  ...
                  className="flex items-center gap-3 p-2 bg-neutral-50 border border-neutral-200 rounded-lg"
                >
                  ...
                  {upload.status === 'error' && (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => removeUpload(upload.file)}>
                        <X className="w-4 h-4 text-neutral-400 hover:text-neutral-900" />
                      </button>
                    </div>
                  )}
