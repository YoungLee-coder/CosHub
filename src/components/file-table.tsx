...
import {
  Folder, File, Image, Video, MoreHorizontal, Download, Trash2, Edit, Search, Loader2, Eye, ArrowUp, ArrowDown, Copy,
} from 'lucide-react'
...
export function FileTable({ bucket, prefix, onNavigate }: FileTableProps) {
  ...
  const handleRename = (file: FileItem) => { setNewName(file.name); setRenameDialog({ open: true, file }) }
  ...
  const columns: ColumnDef<FileItem>[] = [
    {
      ...
      header: ({ table }) => (
        <input type="checkbox" checked={table.getIsAllRowsSelected()}
          onChange={table.getToggleAllRowsSelectedHandler()} className="rounded border-neutral-300" />
      ),
      cell: ({ row }) => (
        <input type="checkbox" checked={row.getIsSelected()}
          onChange={row.getToggleSelectedHandler()} className="rounded border-neutral-300" />
      ),
      size: 40,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const Icon = file.isFolder ? Folder : isImageFile(file.name) ? Image : isVideoFile(file.name) ? Video : File
        return (
          <button onClick={() => file.isFolder && onNavigate(file.key)}
            className={`flex items-center gap-2 ${file.isFolder ? 'hover:text-neutral-600 cursor-pointer' : ''}`}>
            <Icon className={`w-4 h-4 ${file.isFolder ? 'text-neutral-700' : 'text-neutral-400'}`} />
            <span className="truncate max-w-xs">{file.name}</span>
          </button>
        )
      },
    },
    { accessorKey: 'size', header: '大小', cell: ({ row }) => row.original.isFolder ? '-' : formatFileSize(row.original.size) },
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => deleteMutation.mutate(file.key)} className="text-red-600">
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="搜索文件..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)}
        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-neutral-200 hover:bg-transparent bg-neutral-50">
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={5} className="text-center py-8 text-neutral-500">暂无文件</TableCell></TableRow>
            ) : (
